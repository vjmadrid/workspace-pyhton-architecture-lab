...
def run_active_click(driver, element, isactiveClick=False):
    if isactiveClick:
        element.click()
    else:
        driver.execute_script("arguments[0].click();", element)

def retry_find_click(driver, how, what, max_retries, active_refresh, isactiveClick=False):
    """
    Reintenta hasta encontrar el elemento y realiza click sobre el
    """
    ...
    while(num_retries < max_retries):
        ...
        try:
            element = driver.find_element(how, what)
            run_active_click(driver, element, isactiveClick)
            return element
        except StaleElementReferenceException:
            print ("Stale reference on click or find!!!... retrying ")
