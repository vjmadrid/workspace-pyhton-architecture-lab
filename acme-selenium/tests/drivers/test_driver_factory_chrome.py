...
class TestDriverFactoryChrome(unittest.TestCase):
    ...
    def test_get_driver_chrome_options_null(self):
        with pytest.raises(Exception) as excep:
            DriverFactory.get_driver_chrome(None)
        assert "options_dict is an invalid parameter" in str(excep.value)

    def test_get_driver_chrome_invalid_executable_path(self):
        ...
        with pytest.raises(Exception) as excep:
            DriverFactory.get_driver_chrome(TEST_INVALID_OPTIONS_DICT)
    ...
    def test_get_driver_chrome_with_executable(self):
        ...
        browser = DriverFactory.get_driver_chrome(TEST_OPTIONS_WITH_EXECUTABLE_DICT)
        browser.get(testing_google_support.TEST_GOOGLE_URL)
