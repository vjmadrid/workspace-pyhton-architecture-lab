...
class WebElementEx():
    ...
    def __getattribute__(self, item):
        try:
            ...
            return (
                self.element.get_attribute(item)
        ...
        if self._web_element is not None:
            ...
            try:
                ...
                raise NoSuchElementException(
                    msg=f"Unable to locate {self.name} on {self.page} using locator {self.locator}"
                )
    ...
    def all(self):
        try:
            ...
            return [
                ...
                for web_element in result
            ]

        except TimeoutException:
            raise NoSuchElementException(
                msg=f"Unable to locate {self.name} in {self._scope.name} using locator {self.locator}"
            )
    ...
    def is_not_selected(self):
        return Condition(
            ec.element_selection_state_to_be(self.element, False), self._scope
        )

    def has_text(self, text):
        return Condition(
            ec.text_to_be_present_in_element(self.locator, text), self._scope
        )
    ...
    def test(self):
        try:
            with self._setup_test_context():
                assert self._locator.exclude or (
                    bool(self.element) and self.count == self._locator.number
                )
                for key in self.keys:
                    getattr(self, key).test()
        except NoSuchElementException as excep:
            assert False, excep.msg
